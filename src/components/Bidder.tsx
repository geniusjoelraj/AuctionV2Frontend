...
export default function Bidder({ player, finalBid }: { player: Player, finalBid: number }) {
  ...
  return (
    <div className="grid grid-cols-2 grid-rows-5 gap-3 p-4 scale-105">
      ...
      <div className="bg-red-500 p-2 rounded-md hover:bg-red-400 cursor-pointer col-span-2 text-center red-glass-btn" onClick={() => refundPlayer(player.id, gameId)}>refund</div>
    </div>
  )
}
